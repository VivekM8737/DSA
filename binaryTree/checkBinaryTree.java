...
public class checkBinaryTree {
    ...
    public static Fac checkTree(Node node){
        if (node==null) {
            Fac f = new Fac();
            f.max=Integer.MIN_VALUE;
            f.min=Integer.MAX_VALUE;
            f.tbst=true;
            return f;
        }
        Fac rBst=checkTree(node.rch);
        Fac lBst=checkTree(node.lch);
        Fac m=new Fac();
        m.tbst= rBst.tbst && lBst.tbst && (rBst.min>=node.data && lBst.max<=node.data);
        m.min=Math.min(node.data, Math.min(rBst.min, lBst.min));
        m.max=Math.max(node.data, Math.max(rBst.max, lBst.max));
        return m;
    }
    ...
    public static void main(String[] args) {
        ...
        Integer[] arr1 = { 50, 25, 12, null, null, 37, 30, null, null, null, 75, 62, null, 70, null, null, 87, null,
           ...
           System.out.println(checkTree(root).tbst);
       }
}
